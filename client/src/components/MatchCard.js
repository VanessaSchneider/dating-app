...
function MatchCard ({
  ...
  const myMatchArray =
    ...
    filteredProfiles.filter(
      p => p.id === match.like.user_id || p.id === match.like.liked_person_id
    )
  ...
  const messageDisplay = messageList.map(m => (
    <p key={m.id}>
      {m.user_id === user.id ? 'me' : myMatch && myMatch.name}: {m.message}
    </p>
  ))
      ...
      <div className='card text-center' style={{ width: '90rem' }}>
        {myMatch ? (
          <div>
            ...
            {match.messages.length === 0 && openMessage ? (
            ...
            >
              {openMessage ? 'Close Messages' : 'Open Messages'}
            </button>
            {unmatchButton ? (
              <button
                className='btn btn-outline-secondary'
                onClick={() => setUnmatchButton(false)}
              >
                Unmatch
              </button>
            ) : null}
